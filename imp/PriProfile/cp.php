...
// mysql_select_db("placement"); // Selecting Database from Server
	...
	if($Password && $repassword && $cur)
	{
		...
		{
			$sql = $connect->query("SELECT * FROM administrator WHERE name='$Username'");
			if($sql->num_rows== 1)
			{
				$row = $sql->fetch_assoc();
				...
				if($cur == $dbpassword)
				{
					if($query = $connect->query("UPDATE administrator SET password = '$Password' WHERE name = '$Username'"))
					{
...
<html>
<body>
<center><a href="http://localhost/imp/PriProfile/login.php">click here </a></center>
</body>
</html>